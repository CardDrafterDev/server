...
def get_user_data(user_id: int) -> models.User:
    with _db_create_session() as s:
        try:
            ...
            db_handler.handle_misc(
                msg=f"Could not get user data at table {TABLE_NAME}: {e}", e=e
            )
...
def select_all() -> list[models.User]:
    with _db_create_session() as s:
        try:
            ...
            db_handler.handle_misc(
                msg=f"Could not SELECT * at table {TABLE_NAME}: {e}", e=e
            )
            raise e
...
def update_user_inventory(user_id: int, inventory: list[str]) -> None:
    with _db_create_session() as s:
        try:
            data = {"user_id": user_id, "inventory": inventory}
            query_text = text(
                f"""UPDATE {TABLE_NAME} SET inventory=ARRAY{inventory} WHERE user_id={user_id}"""
            )
            s.execute(query_text, data)
            s.commit()
        except Exception as e:
            db_handler.handle_misc(
                msg=f"Could not update inventory at table {TABLE_NAME}: {e}", e=e
            )
            raise e
...
def update_user_collection(user_id: int, collection: list[str]) -> None:
    with _db_create_session() as s:
        try:
            ...
            query_text = text(
                f"""UPDATE {TABLE_NAME} SET collection=ARRAY{collection} WHERE user_id={user_id}"""
            )
            s.execute(query_text, data)
            s.commit()
        except Exception as e:
            db_handler.handle_misc(
                msg=f"Could not update collection at table {TABLE_NAME}: {e}", e=e
            )
            raise e
...
def create_user(user_id: int) -> None:
    with _db_create_session() as s:
        try:
            query_text = text(
                f"""INSERT INTO {TABLE_NAME}(user_id, collection, inventory) VALUES({user_id}, null, null)"""
            )
